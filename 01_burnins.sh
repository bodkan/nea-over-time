...
mkdir data/simulations/burnins
...
for h in 0.0 0.5 1.0; do
    python3 code/run_mutation_accumulation.py \
        ...
        --mut-rate 1e-8 \
        --dominance-coef $h \
        --output data/simulations/burnins/${region}_h_${h}.txt &
done
done
...
mkdir data/simulations/burnins/toy_genome
...
for h in `seq 0.0 0.1 1.0`; do
    python3 code/run_mutation_accumulation.py \
        ...
        --mut-rate 1e-8 \
        --dominance-coef $h \
        --output data/simulations/burnins/toy_genome/independent_haps_h_${h}.txt &
done
python3 code/run_mutation_accumulation.py \
    --regions data/slim_coords/toy_genome/independent_haps_regions.bed \
    --sites data/slim_coords/toy_genome/independent_haps_sites.bed \
    ...
    --mut-rate 0 \
    --dominance-coef 0.0 \
    --output data/simulations/burnins/toy_genome/independent_haps_neutral.txt &
...
for h in `seq 0.0 0.1 1.0`; do
    python3 code/run_mutation_accumulation.py \
        ...
        --mut-rate 1e-8 \
        --dominance-coef $h \
        --output data/simulations/burnins/toy_genome/linked_haps_h_${h}.txt &
done
python3 code/run_mutation_accumulation.py \
    --regions data/slim_coords/toy_genome/linked_haps_regions.bed \
    --sites data/slim_coords/toy_genome/linked_haps_sites.bed \
    ...
    --mut-rate 0 \
    --dominance-coef 0.0 \
    --output data/simulations/burnins/toy_genome/linked_haps_neutral.txt &
