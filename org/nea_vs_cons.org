...
#+BEGIN_SRC R :session :results output graphics :exports both :file ~/devel/slim-neanderthal/org/img/bvalue_bins2.pdf :width 8 :height 5
  n_bins <- 2
  snps <- all_snps %>%
      mutate(bin=ifelse(bStatistic < 200, 1, ifelse(bStatistic > 800, n_bins, NA))) %>%
      filter(!is.na(bin)) %>%
      group_by(bin) %>%
      sample_n(size=min(table(.[["bin"]])))

  qplot(all_snps$bStatistic, bins=100) + geom_vline(xintercept=c(200, 800))
#+END_SRC
...
#+BEGIN_SRC R :session :results output graphics :exports both :file ~/devel/slim-neanderthal/org/img/codingVSintergenic.pdf :width 8 :height 5
...
#+RESULTS:
| EMH                |  0.00116069512417814 |
| America            | 6.11127060266149e-08 |
| CentralAsiaSiberia |    0.146347457854357 |
| EastAsia           |    0.104762604090992 |
| Papuan             |    0.705605417984664 |
| SouthAsia          |     0.93245609852484 |
| WestEurasia        |    0.600027052453275 |
