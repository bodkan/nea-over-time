...
create_qpF4ratio_poplist <- function(X, A, B, C, O, file) {
...
#'     Patterson et al., 2012
#' @param file Path to the poplist file that will be generated.
create_Dstats_poplist <- function(W, X, Y, Z, file) {
    lines <- c()
...
#'     function.
#' @param poplist_file Population list file (Dstats quadruples,
#'     F4ratio populations, etc.).
...
#'     printed to stdout.
run_tool <- function(cmd, param_file, log_file=NULL, admixtools_path="/Users/martin_petr/local/Admixtools/bin/") {
    if (! tool %in% c("qpDstats", "qpF4ratio")) {
